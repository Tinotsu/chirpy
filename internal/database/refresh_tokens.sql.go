...
const getAllRefreshToken = `-- name: GetAllRefreshToken :many
SELECT token, created_at, updated_at, user_id, expires_at, revoked_at FROM refresh_tokens
`

func (q *Queries) GetAllRefreshToken(ctx context.Context) ([]RefreshToken, error) {
	rows, err := q.db.QueryContext(ctx, getAllRefreshToken)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
